/-
  RiscvZkvm.Rv64.Tactics.SymStep
  ...
  `RiscvZkvm.Rv64.step_non_ecall_non_mem` and then unfolds `execInstrBr`,
  ...
  - If `i` is a branch / memory / ECALL / EBREAK instruction, the
    ...
    fail; use the dedicated `step_*` lemmas in `RiscvZkvm/Rv64/Execution.lean`
    directly instead.
  ...
  - `RiscvZkvm/Rv64/Execution.lean` — `step`, `execInstrBr`,
...
meta import Lean
meta import RiscvZkvm.Rv64.Execution
...
macro_rules
  | `(tactic| sym_step $h:ident) =>
    `(tactic|
      first
      | (rw [RiscvZkvm.Rv64.step_non_ecall_non_mem $h
              (by intro hEq; cases hEq)
              (by intro hEq; cases hEq)
              (by simp [RiscvZkvm.Rv64.Instr.isMemAccess])]
         simp only [RiscvZkvm.Rv64.execInstrBr])
      | (simp only [RiscvZkvm.Rv64.step_non_ecall_non_mem $h
                      (by intro hEq; cases hEq)
                      (by intro hEq; cases hEq)
                      (by simp [RiscvZkvm.Rv64.Instr.isMemAccess]),
                    RiscvZkvm.Rv64.execInstrBr]))
...
namespace RiscvZkvm.Rv64.Tactics.SymStepTests
...
end RiscvZkvm.Rv64.Tactics.SymStepTests
