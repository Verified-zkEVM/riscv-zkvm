/-
  ...
  The instruction constructor (e.g., `RiscvZkvm.Rv64.Instr.LW`) is auto-detected
...
structure SpecGenEntry where
  /-- Full name of the instruction constructor (e.g., `RiscvZkvm.Rv64.Instr.ADD`) -/
  instrCtor : Name
...
private meta partial def extractInstrCtorFromType (type : Expr) : Option Name :=
  ...
  | _ =>
    ...
    if type.isAppOfArity `RiscvZkvm.Rv64.cpsTripleWithin 6 then
    ...
    else if type.isAppOfArity `RiscvZkvm.Rv64.cpsBranchWithin 8 then
      let cr := type.getAppArgs[2]!
      let pre := type.getAppArgs[3]!
      findInstrCtorInCodeReq cr |>.orElse fun () => findInstrCtorInPre pre
    -- Try cpsNBranchWithin nSteps addr cr pre exits (5 args)
    else if type.isAppOfArity `RiscvZkvm.Rv64.cpsNBranchWithin 5 then
      let cr := type.getAppArgs[2]!
      let pre := type.getAppArgs[3]!
      findInstrCtorInCodeReq cr |>.orElse fun () => findInstrCtorInPre pre
    -- Try cpsHaltTripleWithin nSteps addr cr pre post (5 args)
    else if type.isAppOfArity `RiscvZkvm.Rv64.cpsHaltTripleWithin 5 then
      let cr := type.getAppArgs[2]!
      let pre := type.getAppArgs[3]!
      findInstrCtorInCodeReq cr |>.orElse fun () => findInstrCtorInPre pre
    else none
