...
`RiscvZkvm/Rv64/Logic/Tactics/ExtractPure.lean`) that strips every `⌜P⌝` leaf from
...
theorem sepConj_pure_right_swap {P : RiscvZkvm.Rv64.Assertion} {Q : Prop} :
    ∀ s, (P ** ⌜Q⌝) s ↔ Q ∧ P s := by
  intro s
  rw [RiscvZkvm.Rv64.sepConj_pure_right]
  exact And.comm
...
private meta partial def collectSepLeaves
  ...
  | (``RiscvZkvm.Rv64.sepConj, #[l, r]) =>
      let (lp, lr) ← collectSepLeaves l
      let (rp, rr) ← collectSepLeaves r
      return (lp ++ rp, lr ++ rr)
  | _ =>
      match e.getAppFnArgs with
      | (``RiscvZkvm.Rv64.pure, _) => return (#[e], #[])
      | _                       => return (#[], #[e])
...
private meta def buildSepChain (xs : Array Expr) : MetaM Expr := do
  if xs.isEmpty then
    return mkConst ``RiscvZkvm.Rv64.empAssertion
  let n := xs.size
  let mut acc := xs[n - 1]!
  for i in [0:n - 1] do
    let j := n - 2 - i
    acc ← mkAppM ``RiscvZkvm.Rv64.sepConj #[xs[j]!, acc]
  return acc
  ...
  | `(tactic| drop_pure $h:ident) => withMainContext do
      ...
      evalTactic (← `(tactic|
        simp only
          ...
          , RiscvZkvm.Rv64.Tactics.sepConj_pure_right_swap
...
namespace RiscvZkvm.Rv64.Tactics.DropPureTests
...
end RiscvZkvm.Rv64.Tactics.DropPureTests
