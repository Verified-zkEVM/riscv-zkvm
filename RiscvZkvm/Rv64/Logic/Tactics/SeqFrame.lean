/-
  RiscvZkvm.Rv64.Tactics.SeqFrame
...
meta import RiscvZkvm.Rv64.Logic.InstructionSpecs
...
meta def parseCpsTripleWithin? (e : Expr) : MetaM (Option (Expr × Expr × Expr × Expr × Expr × Expr)) := do
  ...
  if e.isAppOfArity ``RiscvZkvm.Rv64.cpsTripleWithin 6 then
...
meta def computeFrame (q1 p2 : Expr) : MetaM (List Expr) :=
  ...
  let p2Atoms := p2Atoms.filter fun a => !(a == mkConst ``RiscvZkvm.Rv64.empAssertion)
        ...
        try
          ...
          | some k1Bv, some k2Bv =>
            ...
            if k1 != k2 then
              ...
              return mkApp4 (mkConst ``RiscvZkvm.Rv64.addr_ne_of_bv_ne) base1 k1Bv k2Bv hne
          | none, some kBv =>
            -- base ≠ base + k
            ...
            let hne ← mkDecideProof neType
            return mkApp3 (mkConst ``RiscvZkvm.Rv64.addr_ne_add_right) base1 kBv hne
          | some kBv, none =>
            -- base + k ≠ base
            let bv64Type := mkApp (mkConst ``BitVec) (mkNatLit 64)
            let zeroAddr ← mkNumeral bv64Type 0
            let neType := mkApp3 (mkConst ``Ne [Level.one]) addrType kBv zeroAddr
            let hne ← mkDecideProof neType
            return mkApp3 (mkConst ``RiscvZkvm.Rv64.addr_add_ne_left) base1 kBv hne
...
meta partial def buildPcFreeProof (assertion : Expr) : MetaM Expr := do
  ...
  if e.isAppOfArity ``RiscvZkvm.Rv64.sepConj 2 then
    let l := Expr.appArg! (Expr.appFn! e)
    let r := Expr.appArg! e
    let lPf ← buildPcFreeProof l
    let rPf ← buildPcFreeProof r
    return mkApp4 (mkConst ``RiscvZkvm.Rv64.pcFree_sepConj) l r lPf rPf
  else if e.isAppOfArity `RiscvZkvm.Rv64.instrAt 2 then
    let args := e.getAppArgs
    return mkApp2 (mkConst ``RiscvZkvm.Rv64.pcFree_instrAt) args[0]! args[1]!
  else if e.isAppOfArity `RiscvZkvm.Rv64.regIs 2 then
    let args := e.getAppArgs
    return mkApp2 (mkConst ``RiscvZkvm.Rv64.pcFree_regIs) args[0]! args[1]!
  else if e.isAppOfArity `RiscvZkvm.Rv64.memIs 2 then
    let args := e.getAppArgs
    return mkApp2 (mkConst ``RiscvZkvm.Rv64.pcFree_memIs) args[0]! args[1]!
  else if e.isAppOfArity `RiscvZkvm.Rv64.regOwn 1 then
    return mkApp (mkConst ``RiscvZkvm.Rv64.pcFree_regOwn) e.getAppArgs[0]!
  else if e.isAppOfArity `RiscvZkvm.Rv64.memOwn 1 then
    return mkApp (mkConst ``RiscvZkvm.Rv64.pcFree_memOwn) e.getAppArgs[0]!
  else if e == mkConst ``RiscvZkvm.Rv64.empAssertion then
    return mkConst ``RiscvZkvm.Rv64.pcFree_emp
  else if e.isAppOfArity `RiscvZkvm.Rv64.pure 1 then
    return mkApp (mkConst ``RiscvZkvm.Rv64.pcFree_pure) e.getAppArgs[0]!
  else if e.isAppOfArity `RiscvZkvm.Rv64.publicValuesIs 1 then
    return mkApp (mkConst ``RiscvZkvm.Rv64.pcFree_publicValuesIs) e.getAppArgs[0]!
  else if e.isAppOfArity `RiscvZkvm.Rv64.privateInputIs 1 then
    return mkApp (mkConst ``RiscvZkvm.Rv64.pcFree_privateInputIs) e.getAppArgs[0]!
  else if e.isAppOfArity `RiscvZkvm.Rv64.programAt 1 then
    return mkApp (mkConst ``RiscvZkvm.Rv64.pcFree_programAt) e.getAppArgs[0]!
  else if e.isAppOfArity `RiscvZkvm.Rv64.progAt 2 then
    let args := e.getAppArgs
    return mkApp2 (mkConst ``RiscvZkvm.Rv64.pcFree_progAt) args[0]! args[1]!
  else
    -- Fallback to tactic for unknown assertion types
    let pcFreeType := mkApp (mkConst ``RiscvZkvm.Rv64.Assertion.pcFree) assertion
...
meta def mkPermLambda (src tgt : Expr) : MetaM Expr := do
  ...
  let psType := mkConst ``RiscvZkvm.Rv64.PartialState
...
private meta def buildOfProgDisjointRange (cr1 cr2 : Expr) : MetaM Expr := do
  ...
  let disjType := mkApp2 (mkConst ``RiscvZkvm.Rv64.CodeReq.Disjoint) cr1 cr2
...
meta partial def buildDisjointProof (cr1 cr2 : Expr) : MetaM Expr :=
  ...
  let cr2 ← whnfR cr2
  -- Case: cr1 = empty
  if cr1 == mkConst ``RiscvZkvm.Rv64.CodeReq.empty then
    return ← mkAppM ``RiscvZkvm.Rv64.CodeReq.Disjoint.empty_left #[cr2]
  -- Case: cr2 = empty
  if cr2 == mkConst ``RiscvZkvm.Rv64.CodeReq.empty then
    return ← mkAppM ``RiscvZkvm.Rv64.CodeReq.Disjoint.empty_right #[cr1]
  -- Case: cr1 = singleton a1 i1, cr2 = singleton a2 i2
  if cr1.isAppOfArity ``RiscvZkvm.Rv64.CodeReq.singleton 2 &&
     cr2.isAppOfArity ``RiscvZkvm.Rv64.CodeReq.singleton 2 then
    ...
    return ← mkAppOptM ``RiscvZkvm.Rv64.CodeReq.Disjoint.singleton
  ...
  if cr1.isAppOfArity ``RiscvZkvm.Rv64.CodeReq.singleton 2 &&
     cr2.isAppOfArity ``RiscvZkvm.Rv64.CodeReq.ofProg 2 then
    try
  ...
  if cr1.isAppOfArity ``RiscvZkvm.Rv64.CodeReq.ofProg 2 &&
     cr2.isAppOfArity ``RiscvZkvm.Rv64.CodeReq.singleton 2 then
    try
  ...
  if cr1.isAppOfArity ``RiscvZkvm.Rv64.CodeReq.ofProg 2 then
    ...
    if progW.isAppOfArity ``List.cons 3 then
      ...
      let singletonHead := mkApp2 (mkConst ``RiscvZkvm.Rv64.CodeReq.singleton) base headInstr
      let addrType := mkApp (mkConst ``BitVec) (mkNatLit 64)
      let four ← mkNumeral addrType 4
      let nextBase ← mkAppM ``HAdd.hAdd #[base, four]
      let ofProgTail := mkApp2 (mkConst ``RiscvZkvm.Rv64.CodeReq.ofProg) nextBase rest
      let hd1 ← buildDisjointProof singletonHead cr2
      let hd2 ← buildDisjointProof ofProgTail cr2
      return mkAppN (mkConst ``RiscvZkvm.Rv64.CodeReq.Disjoint.ofProg_cons_left)
        #[base, headInstr, rest, cr2, hd1, hd2]
    else if progW.isAppOfArity ``List.nil 1 then
      return mkApp2 (mkConst ``RiscvZkvm.Rv64.CodeReq.Disjoint.ofProg_nil_left) base cr2
  -- Case: cr2 = ofProg base (i :: rest) → peel head singleton, recurse
  if cr2.isAppOfArity ``RiscvZkvm.Rv64.CodeReq.ofProg 2 then
    ...
    if progW.isAppOfArity ``List.cons 3 then
      ...
      return mkAppN (mkConst ``RiscvZkvm.Rv64.CodeReq.Disjoint.ofProg_cons_right)
        #[cr1, base, headInstr, rest, hd1, hd2]
    else if progW.isAppOfArity ``List.nil 1 then
      return mkApp2 (mkConst ``RiscvZkvm.Rv64.CodeReq.Disjoint.ofProg_nil_right) cr1 base
  -- Fallback: try decide
  let disjType := mkApp2 (mkConst ``RiscvZkvm.Rv64.CodeReq.Disjoint) cr1 cr2
...
meta def mkIdentityMono (cr : Expr) : MetaM Expr := do
  ...
  let instrType := mkConst ``RiscvZkvm.Rv64.Instr
...
private meta def buildMonoProofTactic (oldCr newCr : Expr) : MetaM Expr := do
  ...
  try
    let stx ← `(tactic| intro a i h; simp only [RiscvZkvm.Rv64.CodeReq.singleton, RiscvZkvm.Rv64.CodeReq.union] at *; (first | simp_all | (split at h <;> simp_all <;> bv_omega)))
    runTacticSilent mvar.mvarId! stx
    return ← instantiateMVars mvar
  catch _ =>
...
meta partial def extractUnionChain (cr : Expr) : MetaM (Array (Expr × Expr × Expr)) := do
  let crW ← whnfR cr
  if crW.isAppOfArity ``RiscvZkvm.Rv64.CodeReq.union 2 then
    let head := crW.getAppArgs[0]!
    let tail := crW.getAppArgs[1]!
    let rest ← extractUnionChain tail
    return #[(head, tail, crW)] ++ rest
  else
    return #[(crW, mkConst ``RiscvZkvm.Rv64.CodeReq.empty, crW)]
...
meta def buildMonoProofDirect (oldCr : Expr) (chain : Array (Expr × Expr × Expr))
  ...
  unless oldCrW.isAppOfArity ``RiscvZkvm.Rv64.CodeReq.singleton 2 do return none
  ...
  for i in [:chain.size] do
    ...
    if head.isAppOfArity ``RiscvZkvm.Rv64.CodeReq.singleton 2 then
  ...
  unless matchHead.isAppOfArity ``RiscvZkvm.Rv64.CodeReq.singleton 2 do return none
  ...
  let hitProof := mkApp2 (mkConst ``RiscvZkvm.Rv64.CodeReq.singleton_get) matchAddr matchInstr
  ...
  let mut crProof :=
    if matchCrW.isAppOfArity ``RiscvZkvm.Rv64.CodeReq.union 2 then
      -- union(matchHead, matchTail) at position j
      mkAppN (mkConst ``RiscvZkvm.Rv64.CodeReq.union_hit)
  ...
  while k > 0 do
    ...
    unless skipHead.isAppOfArity ``RiscvZkvm.Rv64.CodeReq.singleton 2 do return none
    ...
    let missProof := mkAppN (mkConst ``RiscvZkvm.Rv64.CodeReq.singleton_miss)
      #[skipAddr, matchAddr, skipHead.getAppArgs[1]!, neqProof]
    crProof := mkAppN (mkConst ``RiscvZkvm.Rv64.CodeReq.union_skip)
  ...
  return some (mkAppN (mkConst ``RiscvZkvm.Rv64.CodeReq.singleton_mono)
...
private meta def buildMonoProofOfProg (oldCrW : Expr) (newCrBase newCrProg : Expr) : MetaM (Option Expr) := do
  ...
  let ofProgExpr := mkApp2 (mkConst ``RiscvZkvm.Rv64.CodeReq.ofProg) newCrBase newCrProg
  if idx == 0 then
    ...
    let lookupProof := mkApp3 (mkConst ``RiscvZkvm.Rv64.CodeReq.ofProg_lookup_zero)
      newCrBase headInstr restList
    let monoProof := mkAppN (mkConst ``RiscvZkvm.Rv64.CodeReq.singleton_mono)
    ...
    let h_addrProof ← do
      ...
      runTacticSilent mvar.mvarId! stx
      instantiateMVars mvar
    let lookupProof := mkAppN (mkConst ``RiscvZkvm.Rv64.CodeReq.ofProg_lookup_addr)
...
private meta def buildMonoProofOfProgToOfProg (oldCrW : Expr)
  ...
  unless oldCrW.isAppOfArity ``RiscvZkvm.Rv64.CodeReq.ofProg 2 do return none
  ...
  let instrTy := mkConst ``RiscvZkvm.Rv64.Instr
  ...
  return some (mkAppN (mkConst ``RiscvZkvm.Rv64.CodeReq.ofProg_mono_sub)
...
meta partial def buildMonoProof (oldCr newCr : Expr) : MetaM Expr :=
  ...
  if newCrW.isAppOfArity ``RiscvZkvm.Rv64.CodeReq.ofProg 2 then
    ...
    if oldCrW.isAppOfArity ``RiscvZkvm.Rv64.CodeReq.union 2 then
      ...
      return ← mkAppM ``RiscvZkvm.Rv64.CodeReq.union_split_mono #[headMono, tailMono]
  ...
  if newCrW.isAppOfArity ``RiscvZkvm.Rv64.CodeReq.union 2 then
    let head := newCrW.getAppArgs[0]!
    let tail := newCrW.getAppArgs[1]!
    -- Left match: oldCr ≡ head (pre-whnfR, preserving abbrev identity)
    if oldCr == head then
      return ← mkAppOptM ``RiscvZkvm.Rv64.CodeReq.union_mono_left #[some head, some tail]
    ...
    try
      ...
      return ← mkAppM ``RiscvZkvm.Rv64.CodeReq.mono_union_right #[disjProof, tailMono]
    catch _ =>
...
meta def seqFrameWithinCore (h1Expr h2Expr : Expr) : MetaM Expr :=
  ...
  let p2IsEmp := preP2N == mkConst ``RiscvZkvm.Rv64.empAssertion
  ...
  if frameAtoms.isEmpty then
    ...
    if ← withoutModifyingState (isDefEq cr1 cr2) then
      return mkAppN (mkConst ``RiscvZkvm.Rv64.cpsTripleWithin_seq_perm_same_cr)
        #[nSteps1, nSteps2, entry, mid1, exit_, cr1, preP, postQ1, preP2, postQ2,
          hperm, h1Expr, h2Expr]
    let hdProof ← buildDisjointProof cr1 cr2
    let h1Perm := mkAppN (mkConst ``RiscvZkvm.Rv64.cpsTripleWithin_weaken)
      #[nSteps1, entry, mid1, cr1, preP, preP, postQ1, preP2,
        (← mkIdLambda preP), hperm, h1Expr]
    return mkAppN (mkConst ``RiscvZkvm.Rv64.cpsTripleWithin_seq)
  ...
  let h2Framed := mkAppN (mkConst ``RiscvZkvm.Rv64.cpsTripleWithin_frameR)
    #[nSteps2, mid2, exit_, cr2, preP2, postQ2, frameExpr, pcFreeProof, h2Expr]

  if p2IsEmp then
    let empStarFrame := mkApp2 (mkConst ``RiscvZkvm.Rv64.sepConj) preP2 frameExpr
    let preRw := mkApp (mkConst ``RiscvZkvm.Rv64.sepConj_emp_left') frameExpr
    let psType := mkConst ``RiscvZkvm.Rv64.PartialState
    ...
    let q2IsEmp := postQ2N == mkConst ``RiscvZkvm.Rv64.empAssertion
    let q2StarFrame := mkApp2 (mkConst ``RiscvZkvm.Rv64.sepConj) postQ2 frameExpr
    let (actualPost, hpost) ← if q2IsEmp then do
      let postRw := mkApp (mkConst ``RiscvZkvm.Rv64.sepConj_emp_left') frameExpr
    ...
    let h2Simplified := mkAppN (mkConst ``RiscvZkvm.Rv64.cpsTripleWithin_weaken)
  ...
  let p2StarFrame := mkApp2 (mkConst ``RiscvZkvm.Rv64.sepConj) preP2 frameExpr
  ...
  let result' ←
    if ← withoutModifyingState (isDefEq gCr rCr) then
      ...
      let monoProof ← try buildMonoProof rCr gCr
          ...
          throw e
      mkAppM ``RiscvZkvm.Rv64.cpsTripleWithin_extend_code #[monoProof, result]
  let result'' ←
...
/-- Lightweight address arithmetic: proves `(a + k₁) + k₂ = a + k₃` via
    ...
    **Prefer `rv64_addr`** (in `RiscvZkvm/Rv64/Logic/AddrNorm.lean`) for new code:
