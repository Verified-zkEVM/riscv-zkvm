...
Ripgrep counts taken on evm-asm's `EvmAsm/` before the relocation
(excluding tactic implementation files):

| Tactic | Implementation file | User call sites under evm-asm's `EvmAsm/` |
|---|---|---|
...
`evm-asm's EvmAsm/Evm64/DivMod/LoopComposeN3.lean`'s
